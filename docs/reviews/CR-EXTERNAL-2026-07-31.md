...
- 较早的完整输出还提到两个防御性边界: 同一迭代内 logical/framebuffer payload 未直接互比，以及宽高超过 `0xffffffff` 时缺少上界校验。这两项均被判断为当前 Intel HiDPI 路径不可达或仅理论风险，不是当前行为回归。
...
OMP 唯一明确提出的缺口是同一迭代内没有直接断言 `logical_payload != framebuffer_payload`。若错误输入把 1x 分辨率送入相似分辨率函数，可能追加两个相同的普通 payload；当前 `intel-hidpi.sh` 生成路径固定使用 `framebuffer = logical * 2`，因此该条件不可达。建议作为防御性改进保留，不应描述为当前 bug。
...
2. logical/framebuffer 同迭代互比缺口属于低风险、防御性、当前不可达建议。它不影响已验证的 1920x1080 BetterDisplay payload 集合一致性。
3. 宽高 32 位上界缺口同样是理论防御项。当前显示器尺寸远低于该边界，现有 framebuffer 上限和正整数校验已阻断现实输入。
4. 本轮不修改业务 `.sh` 或 `.swift`，不改变 `TASKS.md` 状态。
